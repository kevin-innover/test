document.write('<div style="width:'+EDW+';height:11px;text-align:right;margin:-'+(isIE ? 15 : 12)+'px 0 0 -2px;"><img src="'+DTPath+'admin/image/resize.gif" width="11" height="11" style="cursor:n-resize;" onclick="fck_zi();" oncontextmenu="fck_zo();return false;" alt="" title="'+L['fck_zoom']+'"/></div>');
if((UA.indexOf('chrome') != -1 || UA.indexOf('firefox') != -1) && !get_cookie('itip_hide')) document.write('<span style="border:#E7D4AC 1px solid;background:#FFFCDC;padding:2px;color:#666666;" id="itip">'+L['tip_image']+'<a href="javascript:itip_close();void(0);" class="t">'+L['tip_know']+'</a></span>');
function itip_close() {$('#itip').hide();set_cookie('itip_hide', 1);}
...
if(UA.indexOf('chrome') != -1) {
	setTimeout(function() {
		var o = FCKeditorAPI.GetInstance(FCKID);
		...
		d.body.onpaste = function(e) {
			...
			if(e && (clipboardData=e.clipboardData) && (items=clipboardData.items) && (item=items[0]) && item.kind=='file' && item.type.match(/^image\//i)){
				...
				reader.onload=function(){
					if(o.EditMode == FCK_EDITMODE_WYSIWYG) {o.InsertHtml('<img src="'+event.target.result+'">');} else {alert(L['wysiwyg_mode']);}
				}
